...
def compute_final_hash(public_keys):
    """
    Compute the final hash by summing all x and y coordinates of the public keys into a single sum.
    Use modular arithmetic to ensure the total_sum fits within 256 bits.
    """
    MODULO = 2**256  # fit the sum within 256 bits
    total_sum = 0
...
public_keys = [sig["A"] for sig in signature_data]
final_hash = compute_final_hash(public_keys)
